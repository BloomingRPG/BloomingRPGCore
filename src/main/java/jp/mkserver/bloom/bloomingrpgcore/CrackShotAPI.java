...
public class CrackShotAPI {
    ...
    public static void fire(Player player,String parentNode,boolean leftClick){
        ...
        ItemStack item = new CSUtility().generateWeapon(parentNode);
        ...
        for (int burst = burstStart; burst < burstShots; burst++) {
            ...
            int task_ID = Bukkit.getScheduler().scheduleSyncDelayedTask(director, () -> {
                ...
                ItemStack item1 = new CSUtility().generateWeapon(parentNode);


                if (!oneTime) {
                    if (switchedTheItem(player,item, parentNode) || itemSlot != player.getInventory().getHeldItemSlot()) {
    ...
    public static boolean switchedTheItem(Player player, ItemStack item, String parent_node) {
